...
id1 = which(a$V4>=5)
#id2 = which(a$V4>=10 & a$V4<=20)
a$V4[id1] = ">=5"
#a$V4[id2] = "10-20"

shared_diff = data.frame(table(a$V4))
shared_diff$Var1 = factor(shared_diff$Var1,levels=c("1","2","3","4",">=5"))


pdf("DH_FC.Diff_peaks_shared_by_clusters.001.pdf",height=4,width=3)
ggplot(shared_diff) + geom_col(aes(x=Var1,y=Freq)) +
  geom_text(aes(x=Var1,y=Freq,label=Freq),nudge_y=250)+
