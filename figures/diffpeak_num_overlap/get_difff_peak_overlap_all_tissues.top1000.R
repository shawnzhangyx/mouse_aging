...
getDiffTable = function(tissue) {
  ...
  for (cl in 1:max_cluster) {
    ...
    dat[[cl]]$cluster = cl
    dat[[cl]] = dat[[cl]][order(dat[[cl]]$PValue),]
    # Select the top 1% of peaks as diff. 
    dat[[cl]] =  head(dat[[cl]],1000) #dat[[cl]][which(dat[[cl]]$PValue < quantile(dat[[cl]]$PValue,0.01)),]
  }
...
pdf("all_tissues_number_of_diff_peaks_top1k.pdf",height = 12,width = 10)
ggplot(mt2) +
...
write.csv(mat,"All_tissues.diffpeak_share.table.top1k.csv")
#mat = read.csv("All_tissues.diffpeak_share.table.top1k.csv",row.names=1)
...
setwd("/mnt/tscc/lamaral/projects/Aging/aging_share/figures/diffpeak_num_overlap/")
pdf("all_tissues_jaccard_heat_top1k.pdf", height = 14, width = 16)
