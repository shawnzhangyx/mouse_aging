...
mkdir $out_path/R$rank2/bams $out_path/R$rank2/bigWig.cluster $out_path/R$rank2/bigWig.cluster_age bigWig.cluster_age_rep

python split_bam_files.py \
  --bam-prefix ../../analysis/Yang_NMF_method/$tissue/R$rank1/bams/${tissue}.metacell_3. \
  --bam-suffix .bam \
  --statH $out_path/R$rank2/${tissue}.R$rank2.barcode.cluster.stage.rep.txt -o $out_path/R$rank2/bams/${tissue}
...
for i in $(seq 1 $rank2); do
    (
    echo $i
    samtools merge -f $out_path/R$rank2/bams/${tissue}.metacell_${i}.bam $out_path/R$rank2/bams/${tissue}.metacell_${i}.??.rep?.bam

    samtools sort -m 4G $out_path/R$rank2/bams/${tissue}.metacell_${i}.bam -o $out_path/R$rank2/bams/${tissue}.metacell_${i}.sorted.bam
    samtools index $out_path/R$rank2/bams/${tissue}.metacell_${i}.sorted.bam
    ) &
    done
...
for i in $(seq 1 $rank2); do
    bamCoverage --bam $out_path/R$rank2/bams/${tissue}.metacell_${i}.sorted.bam --outFileFormat bigwig --outFileName $out_path/R$rank2/bigWig.cluster/${tissue}.metacell_${i}.sorted.rpkm.bw --binSize 25 --normalizeUsingRPKM
    done
