...
filt = reads[which(reads$reads>7.5e5 & reads$age !="10"),]
...
system("mkdir bam.subsample")
library(doParallel)
registerDoParallel(cores=10)

foreach( idx=1:nrow(filt2) )%dopar% {
   system(paste("samtools view -s ", 7.5e5/filt2$reads[idx], paste0("../",filt2$files[idx]), paste0("-o bam.subsample/",filt2$sample[idx],".bam"  ) ) )
               }
