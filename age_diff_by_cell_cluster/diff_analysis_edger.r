...
reads = read.delim("select_celltypes.subsample.counts",skip=1)

cnts = reads[,-c(1:6)]

ct = sub("bam.subsample.(.*).(..).rep..bam","\\1",colnames(cnts))
age = sub("bam.subsample.(.*).(..).rep..bam","\\2",colnames(cnts))
## unique cell types)
cts = unique(ct)

system("mkdir age_diff_edgeR")
for (ict in cts) {
print(ict)
...
write.csv(out, paste0("age_diff_edgeR/",ict,".edger.txt"))
}

sig_list = NULL
for (ict in cts) {
tmp = read.csv(paste0("age_diff_edgeR/",ict,".edger.txt"))
...
pdf("de_peaks_by_class_tissue.pdf",height=8,width=12)
...
pdf("de_peaks_by_celltypes.pdf",height=6,width=12)
