...
setwd("/projects/ps-renlab/lamaral/projects/aging_RNA/FC/analysis/edgeR")

fs2 = list.files(pattern="03vs18.txt")

dat.list = list()

for (file in fs2) {
  dat.list[[file]] = data.frame(fread(file))[,-c(2:5)]
  dat.list[[file]]$ct = sub("(.*)_03vs18.txt","\\1",file)
    }
...
rna$direction = ifelse(rna$logFC>0,"up","down")
colnames(rna)[1] = "Geneid"
genes = unique(rna$Geneid)
#library(Seurat)
#pbmc = readRDS("/projects/ps-renlab/lamaral/projects/aging_RNA/FC/analysis/DH_seurat_rmdoub_filtered.rds")
...
#  ctrl_gene_list[[length(ctrl_gene_list)+1]] = ct.rna 
  ...
  concord.rna.links = merge(concord.rna[,c(1,2,3,6,7,8)],concord.links[,-c(2:11)],by="Geneid")
...
colnames(concord.out) = c("peak","gene","rna.fc","rna.logCPM","rna.p.adj","ct","rna.direction","cor.fdr","GenePos","atac.fc","atac.logP")
concord.out = concord.out[,c(1,2,9,6,7,3,4,5,8,10,11)]
