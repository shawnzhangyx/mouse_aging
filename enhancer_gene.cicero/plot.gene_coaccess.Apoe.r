...
tss.list = read.table("DH/DH_peak_coaccess_toGene.25.bed",stringsAsFactors=F)
tss = unique(tss.list[tss.list$V5 ==gene,"V4"])
#if (length(tss

conns = read.csv("DH/DH.cicero_conns.25.csv",row.names=1,stringsAsFactors=F)
...
pdf("Apoe.DH.conns.pdf",width=10,height=5)
plot_connections(conns2, "chr7", 19630000, 19790000, 
