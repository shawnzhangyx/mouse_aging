...
load(RData)
system("mkdir -p ../age_diff_edgeR.control/")
...
sample_rep = substr(colnames(mat),4,10)
grp_tab = data.frame(sample_rep=c("03_rep1","10_rep1","18_rep1","03_rep2","10_rep2","18_rep2"),grps = factor(c(1,3,2,2,3,1)))
grps = grp_tab$grps[match(sample_rep,grp_tab$sample_rep)]
...
write.csv(out, paste0("../age_diff_edgeR.control/",cl,".edger.txt"))
}

## write differential peaks into bed files. 
for (cl in 1:max_cluster) {
  print(cl)
  tmp = read.csv(paste0("../age_diff_edgeR.control/",cl,".edger.txt"))
...
#  write.table(out, paste0("../age_diff_edgeR.control/",cl,".both.bed"),row.names=F,col.names=F,quote=F,sep="\t")
  write.table(subset(out,sig.logFC>0), paste0("../age_diff_edgeR.control/",cl,".up.bed"),row.names=F,col.names=F,quote=F,sep="\t")
  write.table(subset(out,sig.logFC<0), paste0("../age_diff_edgeR.control/",cl,".down.bed"),row.names=F,col.names=F,quote=F,sep="\t")
