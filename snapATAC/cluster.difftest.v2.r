###########################################################
## Perform differential binding analysis for every cluster.
##

tissue=commandArgs(trailing=T)[1]
rank=commandArgs(trailing=T)[2]
setwd(paste0("../../analysis/snapATAC/",tissue))
system("mkdir age_diff_bycluster")
a=read.delim(paste0(tissue,".peaks.counts"),skip=1)
...
summ = foreach(grp=unique(grps)) %dopar% {
idx = which(grps==grp)
cnts2 = cnts[,idx]
y = DGEList(cnts2)
...
write.table(out,paste0("age_diff_bycluster/",tissue,".",grp,".peaks.age_diff.txt"),sep="\t",quote=F)
...
up = sub("peak.*:(.*):(.*)-(.*)","\\1\t\\2\t\\3",up)
 if (length(up) > 50) { write(up, paste0("age_diff_bycluster/",tissue,".",grp,".peaks.up.bed")) }
down = rownames(out)[which(out$logFC.3<0 & out$fdr<0.05)]
down = sub("peak.*:(.*):(.*)-(.*)","\\1\t\\2\t\\3",down)
 if (length(down) > 50) { write(down, paste0("age_diff_bycluster/",tissue,".",grp,".peaks.down.bed")) }
comb = c(up,down) 
 if (length(comb) > 50) { write(comb, paste0("age_diff_bycluster/",tissue,".",grp,".peaks.comb.bed")) }
...
write.table(summ2,paste0("age_diff_bycluster/",tissue,".peaks.age_diff.summary.txt"),sep="\t",quote=F,row.names=F)
