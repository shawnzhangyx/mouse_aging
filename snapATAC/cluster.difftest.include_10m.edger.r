...
load(RData)
system("mkdir -p ../age_diff_edgeR.snap.03m.vs.10m/ ../age_diff_edgeR.snap.10m.vs.18m")
...
write.csv(out, paste0("../age_diff_edgeR.snap.03m.vs.10m/",cl,".edger.txt"))
...
write.csv(out, paste0("../age_diff_edgeR.snap.10m.vs.18m/",cl,".edger.txt"))

}

## write differential peaks into bed files. 
for (cl in 1:max_cluster) {
  for ( comp in c("03m.vs.10m","10m.vs.18m")) {
  print(cl)
  tmp = read.csv(paste0("../age_diff_edgeR.snap.",comp,"/",cl,".edger.txt"))
...
#  write.table(out, paste0("../age_diff_edgeR.snap/",cl,".both.bed"),row.names=F,col.names=F,quote=F,sep="\t")
  write.table(subset(out,sig.logFC>0), paste0("../age_diff_edgeR.snap.", comp,"/",cl,".up.bed"),row.names=F,col.names=F,quote=F,sep="\t")
  write.table(subset(out,sig.logFC<0), paste0("../age_diff_edgeR.snap.",comp,"/",cl,".down.bed"),row.names=F,col.names=F,quote=F,sep="\t")
